...
type ProductoCompat = ProductoConVariantes & {
  precio?: unknown;
...
function marginPct(precioBase: number, precioCosto: number): number | null {
  if (!Number.isFinite(precioBase) || !Number.isFinite(precioCosto)) return null;
...
function getProductoPrecioBase(producto: ProductoConVariantes): unknown {
  const productoCompat = producto as ProductoCompat;
  return productoCompat.precioBase ?? productoCompat.precio;
}

function getProductoPrecioCosto(producto: ProductoConVariantes): unknown {
  const productoCompat = producto as ProductoCompat;
  return productoCompat.precioCosto ?? productoCompat.precioCompra;
}

function getProductoIva(producto: ProductoConVariantes): unknown {
  const productoCompat = producto as ProductoCompat;
  return productoCompat.iva ?? productoCompat.impuesto;
}
...
function calcVariantePrecioVenta(variante: IVariante, producto: ProductoConVariantes): number {
  ...
  return toSafeNumber(getProductoPrecioBase(producto)) + toSafeNumber(variante.precioExtra);
}
...
function calcVariantePrecioCosto(variante: IVariante, producto: ProductoConVariantes): number {
  ...
  if (!Number.isNaN(override) && override >= 0) return override;
  return toSafeNumber(getProductoPrecioCosto(producto));
}
...
function ProductoModal({
  ...
  const [form, setForm] = useState<ProductoForm>(() => {
    ...
    return {
      ...
      precioBase: String(toSafeNumber(getProductoPrecioBase(producto))),
      precioCosto: String(toSafeNumber(getProductoPrecioCosto(producto))),
      iva: String(toSafeNumber(getProductoIva(producto))),
    };
  });
...
function VariantesModal({
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingVarianteId) {
        ...
        await api.post('/variantes', { ...payloadBase, productoId });
      }
    },
    onSuccess: () => {
  ...
  const triggerSkuQuick = () =>
    setForm((prev) => ({ ...prev, sku: generateReadableSkuSeed(producto, prev.nombre) }));
...
export default function ProductosPage() {
  ...
  const stats = useMemo(() => {
    ...
    const margenes = productos
      .map((p) =>
        marginPct(
          toSafeNumber(getProductoPrecioBase(p), Number.NaN),
          toSafeNumber(getProductoPrecioCosto(p), Number.NaN),
        ),
      )
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3">
          {[
            ...
            { label: 'Marcas', value: stats.marcas, accent: '#785d4a' },
            {
              ...
              value: stats.margenPromedio === null ? '-' : `${stats.margenPromedio}%`,
              accent:
            ...
            <>
              ...
              <div className="divide-y" style={{ borderColor: 'rgba(218,192,197,0.35)' }}>
                {productos.map((producto) => {
                  const precioBase = parseFiniteNumber(getProductoPrecioBase(producto));
                  const precioCosto = parseFiniteNumber(getProductoPrecioCosto(producto));
                  const iva = parseFiniteNumber(getProductoIva(producto));
                  const margen = marginPct(precioBase ?? Number.NaN, precioCosto ?? Number.NaN);
                  const variantes = producto.variantes ?? [];
                    ...
                    >
                      ...
                      <div>
                        <p className="text-sm font-black" style={{ color: S.textStrong }}>
                          {precioBase === null ? '-' : cop.format(precioBase)}
                        </p>
                        <p className="text-xs" style={{ color: S.textMuted }}>
                          Costo {precioCosto === null ? '-' : cop.format(precioCosto)}
                        </p>
                      </div>
                      <div>
                        <p
