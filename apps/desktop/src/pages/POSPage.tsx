...
function ModalCobro({
  ...
  setSplitPago: (s: SplitPago) => void;
  metodoPago: MetodoPago;
  setMetodoPago: (m: MetodoPago) => void;
  setUsarSplit: (b: boolean) => void;
  onCobrar: () => void;
  onClose: () => void;
  loading: boolean;
}) {
  ...
  const splitMetodos: Array<{ key: keyof SplitPago; label: string; icon: string }> = [
                ...
                <div className="space-y-2">
                  ...
                  {Number(recibido.replace(/[^0-9]/g, '')) > 0 && (
                    <div className="flex justify-between px-1">
                      ...
                      <span className="text-xl font-black" style={{ color: S.primary }}>
                        {cop.format(cambio)}
                      </span>
                    </div>
                  )}
            ...
            <div className="space-y-3">
              {splitMetodos.map((m) => (
                <div key={m.key} className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-bold w-28" style={{ color: S.secondary }}>
                    {m.label}
                  </span>
                  <input
                    type="number"
                    min={0}
                    placeholder="0"
                    value={splitPago?.[m.key] || ''}
                    onChange={(e) =>
                      setSplitPago({
                        ...(splitPago ?? { efectivo: 0, tarjeta: 0, transferencia: 0 }),
              ...
              <div className="flex justify-between pt-2 border-t" style={{ borderColor: S.border }}>
                ...
                <span
                  ...
                  style={{ color: splitOk ? '#2d6a4f' : S.error }}
                >
                  {cop.format(splitTotal)}{' '}
                  {splitOk ? '✓' : `(faltan ${cop.format(total - splitTotal)})`}
                </span>
              </div>
            </div>
          )}
              ...
              <div className="text-right">
                ...
                {(v.stockDisponible ?? 1) === 0 && (
                  <span
                    ...
                    style={{ backgroundColor: S.errorBg, color: S.error }}
                  >
                    Agotado
                  </span>
                )}
...
export default function POSPage() {
  ...
  const [variantesModal, setVariantesModal] = useState<
  ...
  const crearVentaMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        ...
        splitPago: usarSplit
          ? {
              efectivo: splitPago?.efectivo ?? 0,
              tarjeta: splitPago?.tarjeta ?? 0,
              transferencia: splitPago?.transferencia ?? 0,
            }
          : null,
        observaciones,
  ...
  const handleClickProducto = (variantes: (IVariante & { stockDisponible?: number })[]) => {
    if (variantes.length === 1) {
      void handleAgregarVariante(variantes[0]);
    } else {
      setVariantesModal(variantes);
      setShowModalVariantes(true);
    }
  };
  ...
  const productoCards = useMemo(() => {
    const map = new Map<
      string,
      { nombre: string; categoria: string; variantes: (IVariante & { stockDisponible?: number })[] }
    >();
    for (const v of variantesQuery.data ?? []) {
      if (!map.has(v.productoId))
    ...
    <AppLayout>
      {showModalCobro && (
        <ModalCobro
          ...
          usarSplit={usarSplit}
          splitPago={splitPago}
          setSplitPago={setSplitPago}
          metodoPago={metodoPago}
          setMetodoPago={setMetodoPago}
          setUsarSplit={setUsarSplit}
          onCobrar={() => crearVentaMutation.mutate()}
          onClose={() => setShowModalCobro(false)}
          loading={crearVentaMutation.isPending}
        />
      )}
      {showModalVariantes && (
        <ModalVariantes
          ...
          <div className="mb-6 flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1">
              <span
                ...
                style={{ color: S.secondary, fontSize: 20 }}
              >
                search
              </span>
              <input
          ...
          >
            ...
            {search.length > 1 && !variantesQuery.isLoading && productoCards.length === 0 && (
              <div className="col-span-3 flex flex-col items-center justify-center py-16 gap-3">
                ...
                <p className="text-sm font-bold" style={{ color: S.secondary }}>
          ...
          >
            {items.length === 0 && (
              <div className="flex flex-col items-center justify-center h-40 gap-2">
                <span className="material-symbols-outlined text-4xl" style={{ color: S.border }}>
                  shopping_cart
                </span>
                <p className="text-xs font-bold" style={{ color: S.secondary }}>
                  Carrito vacío
                </p>
              </div>
            )}
            {items.map((item) => {
              const lineTotal =
                item.precioUnitario * item.cantidad * (1 - item.descuentoItem / 100);
              return (
                <div
                  key={item.varianteId}
                  className="space-y-3"
            ...
            <div className="mb-6 mt-3 space-y-3 px-1">
              ...
              {totales.descuentoItems > 0 && (
                <div className="flex justify-between text-xs" style={{ color: S.primary }}>
                  <span>Dto. ítems</span>
                  <span>-{cop.format(totales.descuentoItems)}</span>
                </div>
              )}
              {totales.descuentoGlobalMonto > 0 && (
                <div className="flex justify-between text-xs" style={{ color: S.primary }}>
                  <span>Dto. global</span>
                  <span>-{cop.format(totales.descuentoGlobalMonto)}</span>
                </div>
              )}
              <div className="flex justify-between text-xs" style={{ color: S.secondary }}>
                <span>IVA 19%</span>
