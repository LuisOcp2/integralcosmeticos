...
function normalizeCliente(raw: any): ICliente {
  return {
    id: raw.id,
    nombre: raw.nombre ?? '',
    apellido: raw.apellido ?? '',
    documento: raw.documento ?? raw.numeroDocumento ?? '',
    tipoDocumento: raw.tipoDocumento ?? 'CC',
    email: raw.email ?? undefined,
    telefono: raw.telefono ?? raw.celular ?? undefined,
    direccion: raw.direccion ?? undefined,
    fechaNacimiento: raw.fechaNacimiento ? new Date(raw.fechaNacimiento) : undefined,
    puntosFidelidad: Number(raw.puntosFidelidad ?? raw.puntos ?? 0),
...
async function getClientes(): Promise<ICliente[]> {
  ...
  const paginated = data as Paginated<unknown>;
  return (paginated.items ?? []).map(normalizeCliente);
}
