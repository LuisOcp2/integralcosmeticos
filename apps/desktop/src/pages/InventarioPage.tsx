  ...
  const mutation = useMutation({
    mutationFn: async () => {
      await api.post('/inventario/ajuste', {
        varianteId: stock.varianteId, sedeId: stock.sedeId,
        cantidad: Number(cantidad), motivo, nota,
      });
    },
    onSuccess: onSaved,
  });
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={onClose} className="flex-1 py-3 rounded-xl font-bold text-sm border-2 border-outline-variant text-secondary hover:bg-surface-container transition-all">Cancelar</button>
            <button onClick={() => mutation.mutate()} disabled={mutation.isPending}
              className="flex-1 py-3 rounded-xl font-black text-sm text-white disabled:opacity-60 transition-all" style={{ backgroundColor: '#2a1709' }}>
...
export default function InventarioPage() {
  ...
  const exportCSV = () => {
    ...
    const rows = stockFiltrado.map((s) => [s.varianteId, String(s.cantidad), String(s.stockMinimo), s.cantidad <= s.stockMinimo ? 'SI' : 'NO']);
      ...
      <div className="space-y-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-secondary font-medium mt-1">Stock por sede y alertas de reposición</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <select value={sedeId} onChange={(e) => setSedeId(e.target.value)}
              ...
              {(sedesQuery.data ?? []).map((s) => <option key={s.id} value={s.id}>{s.nombre} ({s.ciudad})</option>)}
            </select>
            {sedeId && (
              <>
            ...
            {stockQuery.data && (
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-surface-container-low p-5 rounded-2xl border-l-4 border-primary">
                  <p className="text-xs font-bold text-secondary uppercase tracking-widest mb-1">Productos en {sedeSeleccionada?.nombre}</p>
            ...
            <div className="overflow-hidden rounded-2xl shadow-sm border border-outline-variant/10">
              {stockQuery.isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  <span className="material-symbols-outlined text-5xl text-outline">check_circle</span>
                      ...
                      return (
                        <tr key={stock.id} className={`border-b border-outline-variant/5 ${i % 2 === 0 ? 'bg-surface-container-lowest' : 'bg-surface-container-low'}`}>
                          ...
                          <td className="px-6 py-4 text-right">
                            <span className={`text-lg font-black ${alerta ? 'text-error' : 'text-on-surface'}`}>{stock.cantidad}</span>
                          </td>
