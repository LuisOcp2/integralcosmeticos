...
export default function CajaPage({ user, sedeId, onOpenPos, onOpenUsuarios }: CajaPageProps) {
  ...
  const esperado = Number(cajaActiva?.montoInicial ?? 0) + Number(cajaActiva?.totalEfectivo ?? 0);
                ...
                {historialQuery.isLoading ? (
                  ...
                  <div className="space-y-3">
                    {(historialQuery.data ?? []).slice(0, 5).map((sesion) => (
                      <div
                        key={sesion.id}
                        className="rounded-2xl border border-outline-variant p-3"
                      >
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                          <div className="inline-flex items-center gap-1 text-on-surface-variant">
                            ...
                            {formatCOP(Number(sesion.montoInicial))}
                          </div>
                          <div className="inline-flex items-center gap-1 text-on-surface-variant">
                            <Banknote className="w-3.5 h-3.5" /> Efec:{' '}
                            {formatCOP(Number(sesion.totalEfectivo))}
                          </div>
                          <div className="inline-flex items-center gap-1 text-on-surface-variant">
                            <CreditCard className="w-3.5 h-3.5" /> Ventas:{' '}
                            {formatCOP(Number(sesion.totalVentas))}
                          </div>
